class Rules:
    ...
    def _already_exists(self, ref_task: str) -> bool:
        """Return True if an identical task string is already in the task list."""
        return any(v == ref_task for v in self.task.values())

    def generate_task(self, ref_task: str):
        if self._already_exists(ref_task):
            return
